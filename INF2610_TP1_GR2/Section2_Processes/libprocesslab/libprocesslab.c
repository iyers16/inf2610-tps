...
{
    int i;
    if (procLevel < 0 || procLevel > _LEVELS)
        return;
    if (procLevel == 1 && (procNum < 1 || procNum > _NB_1))
        return;
    if (procLevel == 2 && (procNum < 1 || procNum > _NB_2))
        return;
    if (procLevel == 0)
    {
        pseg = allocateSharedMemory(_ALL);
        i = 0;
    }
    if (procLevel == 1)
        i = procNum;
    ...
    if (pseg == NULL)
        return;
    pseg[i].procLevel = procLevel;
    pseg[i].procNum = procNum;
    pseg[i].pid = pid;
    pseg[i].ppid = ppid;
}
